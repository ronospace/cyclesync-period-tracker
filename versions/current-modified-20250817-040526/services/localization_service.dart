...
class LocalizationService extends ChangeNotifier {
  ...
  // Force English for now - can be expanded later
  static const bool _forceEnglish = true;
  ...
  Future<void> initialize() async {
    try {
      ...
      if (_forceEnglish) {
        // Force English for now - this ensures consistent UI
        _currentLocale = const Locale('en', 'US');
      } else if (savedLocaleString != null) {
        final parts = savedLocaleString.split('_');
        if (parts.length == 2) {
          final locale = Locale(parts[0], parts[1]);
          if (supportedLocales.contains(locale) && _isSafeLocale(locale)) {
        ...
        if (supportedLocales.contains(systemLocale) && _isSafeLocale(systemLocale)) {
          ...
          // Force English as safe fallback
          _currentLocale = const Locale('en', 'US');
        }
      }
      ...
      _currentLocale = const Locale('en', 'US'); // Always fallback to English
      _isInitialized = true;
      notifyListeners();
    }
  }
  ...
  /// Check if locale has AppLocalizations support (safe to use)
  bool _isSafeLocale(Locale locale) {
    // List of locales that have AppLocalizations (ARB files) support
    // These must match exactly with our generated AppLocalizations
    const safeLanguageCodes = {
