...
class AIPredictionService {
  ...
  static Future<List<PersonalizedInsight>> _generatePersonalizedInsights(
    ...
    if (cycleLengths.isNotEmpty) {
      ...
      insights.add(PersonalizedInsight(
        ...
        severity: regularity > 0.6 ? InsightSeverity.positive : InsightSeverity.neutral,
    ...
    if (commonSymptoms.isNotEmpty) {
      insights.add(PersonalizedInsight(
        ...
        message: 'You commonly experience: ${commonSymptoms.take(3).join(", ")}. '
  ...
  static List<AIRecommendation> _generateRecommendations(
    ...
    recommendations.add(AIRecommendation(
      ...
      description: 'Connect to HealthKit or Google Fit for richer insights including sleep and activity data.',
  ...
  static List<String> _findCommonSymptoms(List<Map<String, dynamic>> cycles) {
    ...
    for (final cycle in cycles) {
      final symptoms = cycle['symptoms'] as List<dynamic>? ?? [];
      for (final symptom in symptoms) {
        symptomCounts[symptom.toString()] = (symptomCounts[symptom.toString()] ?? 0) + 1;
      }
    }
    
    return symptomCounts.entries
        .where((e) => e.value >= cycles.length * 0.3) // Appears in 30%+ of cycles
        .map((e) => e.key)
        .toList();
  }
