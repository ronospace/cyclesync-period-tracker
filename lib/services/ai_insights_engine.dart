...
class AIInsightsEngine {
  ...
  static Future<List<AIInsight>> generatePersonalizedInsights({
    ...
    if (cycles.isEmpty) {
      insights.add(AIInsight(
        ...
        message: 'Start logging your daily symptoms and mood to unlock personalized AI insights.',
    ...
    if (cycles.length >= 3) {
      ...
      if (completedCycles.length >= 2) {
        ...
        insights.add(AIInsight(
          ...
          message: 'Your average cycle length is ${avgLength.toStringAsFixed(1)} days. This information helps predict your next cycle.',
          createdAt: DateTime.now(),
    ...
    if (mostCommonSymptom != null) {
      insights.add(AIInsight(
        ...
        message: 'You frequently experience $mostCommonSymptom. Consider tracking triggers and discussing management options.',
        createdAt: DateTime.now(),
        metadata: {'symptom': mostCommonSymptom, 'frequency': symptomFrequency[mostCommonSymptom]},
      ));
    }
    ...
    if (cycles.isNotEmpty) {
      ...
      if (avgMood < 2.5) {
        insights.add(AIInsight(
          ...
          message: 'Your average mood has been lower lately. Consider stress management techniques or speaking with a healthcare provider.',
          createdAt: DateTime.now(),
        ...
        insights.add(AIInsight(
          ...
          message: 'Your mood has been consistently positive. Keep up the good work with your current routine!',
          createdAt: DateTime.now(),
          metadata: {'average_mood': avgMood},
        ));
      }
  ...
  static Future<AIHealthInsights> generateInsights({
    ...
    final insights = AIHealthInsights(
      ...
      personalizedRecommendations: await _generatePersonalizedRecommendations(cycles, dailyLogs),
  ...
  static Future<List<PersonalizedRecommendation>> _generatePersonalizedRecommendations(
    ...
    if (cycles.isEmpty) {
      recommendations.add(PersonalizedRecommendation(
        ...
        description: 'Begin logging your cycles to unlock personalized insights',
    ...
    if (wellbeingTrends.averageEnergy < 3.0) {
      recommendations.add(PersonalizedRecommendation(
        ...
        description: 'Your energy levels suggest potential sleep quality issues. Consider a consistent sleep routine 30 minutes earlier.',
    ...
    if (wellbeingTrends.averageMood < 3.0) {
      ...
      recommendations.add(PersonalizedRecommendation(
        ...
        priority: moodTrend < -0.5 ? RecommendationPriority.high : RecommendationPriority.medium,
        actionable: true,
        aiConfidence: 0.78,
        suggestedActions: [
    ...
    if (wellbeingTrends.averagePain > 2.5) {
      recommendations.add(PersonalizedRecommendation(
        ...
        description: 'Your pain levels are elevated. These natural approaches might help reduce discomfort.',
    ...
    if (completedCycles < cycles.length * 0.7) {
      recommendations.add(PersonalizedRecommendation(
        ...
        description: 'More complete cycle data will unlock better predictions and insights.',
        category: RecommendationCategory.tracking,
    ...
    if (commonSymptoms.contains('bloating') || commonSymptoms.contains('digestive_issues')) {
      recommendations.add(PersonalizedRecommendation(
        id: 'nutrition_digestive',
        title: 'Digestive Health Support',
        description: 'Your symptoms suggest digestive sensitivity. Consider these dietary adjustments.',
  ...
  static Future<List<PredictiveAlert>> _generatePredictiveAlerts(
    ...
    if (variance > 25) { // High irregularity threshold
      alerts.add(PredictiveAlert(
        id: 'cycle_irregularity',
        title: 'Cycle Irregularity Detected',
        description: 'Your cycles show increasing variability. This could indicate hormonal changes.',
    ...
    if (recentSymptoms.contains('severe_cramps')) {
      ...
      alerts.add(PredictiveAlert(
        ...
        description: 'Based on your patterns, prepare for potential discomfort around your next cycle.',
    ...
    if (moodTrend < -0.3) { // Declining mood trend
      alerts.add(PredictiveAlert(
        id: 'mood_decline',
        title: 'Mood Support Needed',
        description: 'Your mood patterns suggest you might benefit from additional emotional support.',
        severity: AlertSeverity.medium,
  ...
  static Future<List<SymptomInsight>> _analyzeSymptomPatterns(
    ...
    for (final entry in symptomFrequency.entries) {
      ...
      if (percentage >= 30) { // Frequent symptoms (30%+ of cycles)
        final correlatedSymptoms = symptomCorrelations[symptom]
            ?.entries
        ...
        insights.add(SymptomInsight(
          ...
          recommendation: _getSymptomRecommendation(symptom, percentage, correlatedSymptoms),
  ...
  static Future<WellbeingCoaching> _generateWellbeingCoaching(
    ...
    if (cycles.isEmpty) {
      return WellbeingCoaching(
        ...
        motivationalMessage: 'Every health journey begins with a single step. Start tracking today!',
        progressInsights: [],
      );
    }
    ...
    if (cycles.length >= 3) {
      ...
      if (olderCycles.isNotEmpty) {
        final recentAvgMood = recentCycles.map((c) => c.wellbeing.mood).reduce((a, b) => a + b) / recentCycles.length;
        final olderAvgMood = olderCycles.map((c) => c.wellbeing.mood).reduce((a, b) => a + b) / olderCycles.length;
        
        if (recentAvgMood > olderAvgMood + 0.3) {
          progressInsights.add('Your mood has improved by ${((recentAvgMood - olderAvgMood) * 20).round()}% recently!');
        }
      }
    }
    ...
    return WellbeingCoaching(
      weeklyGoals: weeklyGoals.isEmpty ? ['Continue consistent tracking'] : weeklyGoals,
      dailyHabits: dailyHabits.isEmpty ? ['Log your daily symptoms and mood'] : dailyHabits,
      motivationalMessage: motivationalMessages[Random().nextInt(motivationalMessages.length)],
  ...
  static Future<CycleOptimization> _generateCycleOptimization(
    ...
    if (cycles.length < 3) {
      return CycleOptimization(
        ...
        nutritionTiming: {},
        energyPredictions: {},
        lifestyleAdjustments: ['Focus on consistent sleep schedule', 'Stay hydrated throughout your cycle'],
      );
    }
    ...
    if (completedCycles.isEmpty) {
      return CycleOptimization(
        ...
        nutritionTiming: {},
        energyPredictions: {},
        lifestyleAdjustments: ['Complete your cycle tracking for better optimization'],
      );
    }
  ...
  static List<String> _analyzeRecentSymptomTrends(List<CycleData> cycles) {
    ...
    for (final cycle in recentCycles) {
      for (final symptom in cycle.symptoms) {
        if (symptom.severity >= 3) { // Moderate to severe symptoms
  ...
  static DateTime _predictNextCycleStart(List<CycleData> completedCycles) {
    if (completedCycles.isEmpty) return DateTime.now().add(const Duration(days: 28));
    
    final avgLength = completedCycles
        .map((c) => c.lengthInDays)
        .reduce((a, b) => a + b) / completedCycles.length;
  ...
  static SymptomTrend _analyzeSymptomTrend(List<CycleData> cycles, String symptom) {
    final recentCycles = cycles.take(3).toList();
    final olderCycles = cycles.skip(3).take(3).toList();
    
    if (olderCycles.isEmpty) return SymptomTrend.stable;
    
    final recentCount = recentCycles.where((c) => 
        c.symptoms.any((s) => s.name == symptom)).length;
    final olderCount = olderCycles.where((c) => 
        c.symptoms.any((s) => s.name == symptom)).length;
  ...
  static double _analyzeSymptomSeverity(List<CycleData> cycles, String symptom) {
  ...
  static String _getSymptomRecommendation(String symptom, int frequency, List<String> correlatedSymptoms) {
    final recommendations = {
      'cramps': 'Consider heat therapy, gentle exercise, and magnesium supplements.',
      'bloating': 'Try reducing sodium intake and increasing water consumption.',
      'mood_swings': 'Practice mindfulness, maintain stable blood sugar, and ensure adequate sleep.',
      'fatigue': 'Focus on iron-rich foods, regular sleep schedule, and gentle movement.',
      'headache': 'Stay hydrated, manage stress, and monitor trigger foods.',
      'acne': 'Maintain consistent skincare routine and consider dairy reduction.',
    };
    
    final base = recommendations[symptom] ?? 'Monitor patterns and consider lifestyle adjustments.';
  ...
  static Map<String, double> _analyzeEnergyPatterns(List<CycleData> completedCycles) {
    ...
    for (final cycle in completedCycles) {
      ...
      patterns['Menstrual']!.add(cycle.wellbeing.energy * 0.8); // Lower during menstruation
      patterns['Follicular']!.add(cycle.wellbeing.energy * 1.1); // Higher in follicular
      patterns['Ovulatory']!.add(cycle.wellbeing.energy * 1.2); // Highest during ovulation
      patterns['Luteal']!.add(cycle.wellbeing.energy * 0.9); // Moderate in luteal
    }
    
    return patterns.map((phase, energies) => MapEntry(
      phase,
      energies.isEmpty ? 3.0 : energies.reduce((a, b) => a + b) / energies.length,
    ));
  }

  static double _calculateConfidenceScore(List<CycleData> cycles, List<DailyLogEntry> dailyLogs) {
    if (cycles.isEmpty) return 0.1;
    
    double score = 0.0;
    
    // Data quantity score
    final dataQuantityScore = min(1.0, cycles.length / 6.0); // Max score at 6 cycles
    score += dataQuantityScore * 0.4;
    
    // Data completeness score
    final completedCycles = cycles.where((c) => c.isCompleted).length;
    final completenessScore = cycles.isEmpty ? 0.0 : completedCycles / cycles.length;
    score += completenessScore * 0.3;
    
    // Data recency score (higher score for more recent data)
    final now = DateTime.now();
    final daysSinceLastEntry = cycles.isEmpty ? 365 : now.difference(cycles.first.startDate).inDays;
    final recencyScore = max(0.0, 1.0 - daysSinceLastEntry / 90.0); // Max score for entries within 90 days
