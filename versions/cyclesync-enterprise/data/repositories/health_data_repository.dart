...
class HealthDataRepository extends ChangeNotifier {
  ...
  void _setupRealtimeListeners() {
    ...
    FirebaseService.getCyclesStream().listen(
      (cycles) => _handleCyclesUpdate(cycles),
      onError: (error) => debugPrint('❌ Cycles stream error: $error'),
    );
...
class DataSyncStatus {
  ...
  factory DataSyncStatus.success(String message) => DataSyncStatus(
  ...
  factory DataSyncStatus.error(String message) => DataSyncStatus(
    state: SyncState.error,
    message: message,
    timestamp: DateTime.now(),
  );

  factory DataSyncStatus.syncing() => DataSyncStatus(
...
class HealthKitSyncResult {
  ...
  factory HealthKitSyncResult.error(String error) => HealthKitSyncResult(
    success: false,
